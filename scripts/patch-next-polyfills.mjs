...
const stripTrailingSemicolon = (value) => value.replace(/;$/, "");

const stripKnownRemovedSegments = (value) => {
  let remaining = stripTrailingSemicolon(value);

  for (const segment of REMOVED_POLYFILL_SEGMENTS) {
...
try {
  ...
  const normalizedPatchedCandidate = stripKnownRemovedSegments(
    normalizedCurrentPolyfillModule,
  );

  if (
    normalizedPatchedCandidate !==
    stripTrailingSemicolon(PATCHED_POLYFILL_MODULE)
  ) {
    fail(
