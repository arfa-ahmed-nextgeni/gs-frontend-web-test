...
export const useAddPaymentCardForm = ({
  ...
  const submitToken = async (token: string, saveAsDefault: boolean) => {
    const formData = new FormData();
    formData.append(AddPaymentCardFormField.CheckoutComToken, token);
    formData.append(
  ...
  return { isRefreshing, submitToken };
};
