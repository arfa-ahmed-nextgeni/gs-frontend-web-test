...
export class CategoryProductsModel extends Helper {
  products: ProductCardModel[] = [];

  constructor(data: null | ProductSearchQuery, variant: ProductCardVariant) {
    super();

    this.products =
      data?.productSearch.items?.map((item) => {
        ...
        if (product?.__typename === "ComplexProductView") {
          ...
          availableStock = this.toInteger(
            Object.values(associatedProducts)?.[0]?.available_stock ?? undefined
          );
          ...
          currency =
            product?.price?.final?.amount?.currency !== ProductViewCurrency.None
        ...
        const effectiveVariant = isBundlesProductType(productType.value)
        ...
        if (hasProductTags) {
          ...
          badges.push({
            ...this.parseProductTagAttributes(customTag?.tag_attributes || ""),
