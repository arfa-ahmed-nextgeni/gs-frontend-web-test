...
export const getProductDetails = ({
  ...
  PRODUCT_DETAILS_CACHE_DISABLED
    ? getProductDetailsRequestCached(locale, urlKey)
    : getProductDetailsPersistent(locale, urlKey);

const fetchProductDetails = async (locale: Locale, urlKey: string) => {
  try {
...
const getProductDetailsRequestCached = cache(fetchProductDetails);

const getProductDetailsPersistent = cache(
  async (locale: Locale, urlKey: string) => {
    "use cache";
    cacheTag(CacheTags.Catalog, CacheTags.ProductDetails);
    return fetchProductDetails(locale, urlKey);
  }
);
