...
export async function removeCouponFromCartAction() {
  try {
    ...
    const response = await graphqlRequest({
      authToken,
      query: CART_GRAPHQL_MUTATIONS.REMOVE_COUPON_FROM_CART,
    ...
    const cart = response.data?.removeCouponFromCart?.cart;

    if (!cart) {
      return failure("Failed to remove coupon");
    }
