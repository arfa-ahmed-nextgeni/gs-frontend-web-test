...
import { redeemMokafaaPoints } from "@/lib/actions/alrajhi-mokafaa/redeem-points";
...
export const useRedeemMokafaaPoints = () => {
  const queryClient = useQueryClient();
  const locale = useLocale() as Locale;
  const { showOfflineMessage } = useOfflineToast();

  const activeMutations = useIsMutating({
    mutationKey: mutationPrefix(MUTATION_KEYS.MOKAFAA.REDEEM({ locale })),
  });

  return useMutation({
    mutationFn: redeemMokafaaPoints,
    mutationKey: MUTATION_KEYS.MOKAFAA.REDEEM({ locale }),
