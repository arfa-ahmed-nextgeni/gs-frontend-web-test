...
import { authenticateMokafaaCustomer } from "@/lib/actions/alrajhi-mokafaa/authenticate-customer";
...
export const useAuthenticateMokafaaCustomer = () => {
  const queryClient = useQueryClient();
  const locale = useLocale() as Locale;
  const { showOfflineMessage } = useOfflineToast();

  const activeMutations = useIsMutating({
    mutationKey: mutationPrefix(MUTATION_KEYS.MOKAFAA.AUTHENTICATE({ locale })),
  });

  return useMutation({
    mutationFn: authenticateMokafaaCustomer,
    mutationKey: MUTATION_KEYS.MOKAFAA.AUTHENTICATE({ locale }),
